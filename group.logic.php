...
function group_add(CRObject $group)
{
	...
	$res['errno'] = GroupManager::add($group) ? Code::SUCCESS : Code::UNKNOWN_ERROR;
	$log = new CRObject();
...
function group_remove(CRObject $group)
{
	...
	if ($origin === null) {
		...
		$res['errno'] = GroupManager::remove($group) ? Code::SUCCESS : Code::UNKNOWN_ERROR;
	}
	$log = new CRObject();
	$log->set('scope', Session::get('uid'));
...
function group_update(CRObject $group)
{
	...
	if ($origin === null) {
		...
		$res['errno'] = GroupManager::update($group) ? Code::SUCCESS : Code::UNKNOWN_ERROR;
	}
	$log = new CRObject();
	$log->set('scope', Session::get('uid'));
...
function group_gets(CRObject $rule)
{
	...
	$res['errno'] = $res['groups'] === null ? Code::UNKNOWN_ERROR : Code::SUCCESS;
	return $res;
}