...
function contact_add(CRObject $contact)
{
	...
	$res['errno'] = ContactManager::add($contact) ? Code::SUCCESS : Code::UNKNOWN_ERROR;
	$log = new CRObject();
...
function contact_remove(CRObject $contact)
{
	...
	if ($origin === null) {
		...
		$res['errno'] = ContactManager::remove($contact) ? Code::SUCCESS : Code::UNKNOWN_ERROR;
	}
	$log = new CRObject();
	$log->set('scope', Session::get('uid'));
...
function contact_update(CRObject $contact)
{
	...
	if ($origin === null) {
		...
		$res['errno'] = ContactManager::update($contact) ? Code::SUCCESS : Code::UNKNOWN_ERROR;
	}
	$log = new CRObject();
	$log->set('scope', Session::get('uid'));
